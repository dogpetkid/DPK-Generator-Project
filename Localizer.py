...
def sanitizeUnicodeError(string:str):
    ...
    while offset < len(string):
        # the runtime of this is likely poor, but I don't think it matters, nor do I care
        try:
            print(string[offset], file=devnull)
            offset+= 1
        except UnicodeError:
            string = string[:offset] + string[offset+1:]
...
if __name__ == "__main__":
    ...
    for localization in textdatablock.data["Blocks"]:
        print("--------------------------------------------------")
        print(sanitizeUnicodeError(localization["English"]))
